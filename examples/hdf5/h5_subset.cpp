...
int main(void)
{
	...
	file_id = H5Fcreate(FILE, H5F_ACC_TRUNC, H5P_DEFAULT, faplid);  assert(file_id >= 0);
	...
	dataset_id = H5Dcreate2(file_id, DATASETNAME, H5T_STD_I32BE, dataspace_id,
							H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);  assert(dataset_id >= 0);
	...
	status = H5Dwrite(dataset_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL,
					  H5P_DEFAULT, data); assert(status == 0);
	...
	file_id = H5Fopen(FILE, H5F_ACC_RDWR, H5P_DEFAULT);  assert(file_id >= 0);
	dataset_id = H5Dopen2(file_id, DATASETNAME, H5P_DEFAULT);  assert(dataset_id >= 0);
	...
	status = H5Sselect_hyperslab(dataspace_id, H5S_SELECT_SET, offset,
								 stride, count, block); assert(status == 0);
	...
	status = H5Dwrite(dataset_id, H5T_NATIVE_INT, memspace_id,
					  dataspace_id, H5P_DEFAULT, sdata); assert(status == 0);

	status = H5Dread(dataset_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL,
					 H5P_DEFAULT, rdata); assert(status == 0);
	...
	status = H5Sclose(memspace_id); assert(status == 0);
	status = H5Sclose(dataspace_id); assert(status == 0);
	status = H5Dclose(dataset_id); assert(status == 0);
	status = H5Fclose(file_id); assert(status == 0);
