...
herr_t H5VL_log_dataspacei_get_sel_type(hid_t sid, size_t esize, MPI_Datatype *type){
    herr_t err = 0;
    int mpierr;
    int i, j;
    bool derived_etype;
    ...
    etype =  H5VL_log_dtypei_mpitype_by_size(esize);
    if (etype == MPI_DATATYPE_NULL){
        mpierr = MPI_Type_contiguous(esize, MPI_BYTE, &etype); CHECK_MPIERR
    ...
    ndim = H5Sget_simple_extent_dims(sid, hsize, NULL); CHECK_ID(ndim)
    for(i = 0; i < ndim; i++){
        size[i] = (MPI_Offset)hsize[i];
    }

    err = H5VL_log_dataspacei_get_selection(sid, sels); CHECK_ERR
    ...
    for(i = 0; i < n; i++){
        ...
        mpierr = H5VL_log_debug_MPI_Type_create_subarray(ndim, size, ssize, sstart, MPI_ORDER_C, etype, types + i); CHECK_MPIERR
        mpierr = MPI_Type_commit(types + i); CHECK_MPIERR
        offs[i] = 0;
        lens[i] = 1;
    }

    mpierr = MPI_Type_struct(n, lens, offs, types, type); CHECK_MPIERR
    mpierr = MPI_Type_commit(type); CHECK_MPIERR
...
herr_t H5VL_log_dataspacei_get_selection(hid_t sid, std::vector<H5VL_log_selection> &sels){
    herr_t err = 0;
    int i, j, k, l;
    int ndim;
                    ...
                    if (group[i] != group[j]){  // Within 1 group
                        if (i - j == 1){    // Sinlge block, no need to breakdown
                            for(k = 0; k < ndim; k++){
                                ...
                                sels[nreq].count[k] =  (MPI_Offset)(hends[j][k] - hstarts[j][k] + 1);
                            }
                            //offs[k] = nreq; // Record offset
                            nreq++;
                            j++;
                            ...
                            for(;j < i; j++){    // Breakdown each block
                                ...
                                sels[nreq].count[ndim - 1] =  (MPI_Offset)(hends[j][ndim - 1] - hstarts[j][ndim - 1] + 1);
                                ...
                                while(l < ndim - 1){    // While we haven't handle the last one
                                    memcpy(sels[nreq].start, sels[nreq - 1].start, sizeof(MPI_Offset) * ndim);  // Start from previous value
                                    memcpy(sels[nreq].count, sels[nreq - 1].count, sizeof(MPI_Offset) * ndim);  // Fill in Count
