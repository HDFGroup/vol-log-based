...
herr_t H5VL_log_filei_metaflush (H5VL_log_file_t *fp) {
	...
	for (i = 0; i < fp->ndset; i++) {
		if (flag[i] & LOGVOL_META_FLAG_ZIP) {
			...
			clen  = max_mlen;
			err	  = logvol_zip_compress (buf + moffs[i] + sizeof (int) * 3, inlen, zbuf, &clen);
			if ((err == 0) && (clen < inlen)) {
	...
	TIMER_START;
	err = H5VLdataset_optional_wrapper (mdp, fp->uvlid, H5VL_NATIVE_DATASET_GET_OFFSET, fp->dxplid,
										NULL, &mdoff);
	CHECK_ERR
	...
	// MPI file write
	mpierr =
		MPI_File_set_view (fp->fh, (MPI_Offset)mdoff, MPI_BYTE, mdtype, "native", MPI_INFO_NULL);
	CHECK_MPIERR
