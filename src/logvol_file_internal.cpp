...
herr_t H5VL_log_filei_metaflush(H5VL_log_file_t *fp)
{
...
#ifdef ENABLE_ZLIB
            if (cnt[i] > 0)
            { // Compress if we have more than 1k entry
    ...
    for (i = 0; i < fp->ndset; i++)
    {
        if (flag[i] & LOGVOL_META_FLAG_ZIP)
        {
            ...
            clen = max_mlen;
            err =
                logvol_zip_compress(buf + moffs[i] + sizeof(int) * 3, inlen, zbuf, &clen);
            if ((err == 0) && (clen < inlen))
            {
        ...
        // Memory space not contiguous after zip, need selection
        start = moffs[i];
        count = mlens[i];
        err = H5Sselect_hyperslab(mmsid, H5S_SELECT_OR, &start, NULL, &one, &count);
        CHECK_ERR
    }
...
#ifdef LOGVOL_PROFILING
    {
        ...
        H5VL_log_profile_add_time(fp, TIMER_FILEI_METAFLUSH_SIZE_ZIP,
	...
	*/
    ...
    { // Create the idx dataset and look up table dataset
